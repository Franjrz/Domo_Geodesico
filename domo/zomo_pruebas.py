...
class Zomo():
    ...
    def __calcular_todas_curvas(self):
        self.x_pts = [self.x_pts]
        self.y_pts = [self.y_pts]
        for k in range(self.n):
            theta = 2 * np.pi * k / self.n
            cos_t, sin_t = np.cos(theta), np.sin(theta)

            self.x_pts.append(cos_t * self.x_pts[0] - sin_t * self.y_pts[0])
            self.y_pts.append(sin_t * self.x_pts[0] + cos_t * self.y_pts[0])

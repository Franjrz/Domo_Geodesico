...
def visualizar_cara_final(puntos, vertices, aristas):
   """
   Visualiza el triángulo con sus puntos y aristas.
...
def visualizar_cara_final_ids(puntos, vertices, aristas):
    """
    Visualiza el triángulo con sus puntos, aristas y muestra los IDs de los puntos.
    ...
    # Dibujar los puntos interiores y sus IDs
    for p_id in puntos_interiores:
        p = puntos[p_id]
        ax.scatter(p[0], p[1], color=color_puntos_internos, s=40, zorder=4)
        
        # Añadir el ID del punto arriba a la derecha
    ...
    # Dibujar los vértices exteriores en azul oscuro y sus IDs
    color_vertices = '#00008B'  # Azul oscuro (Dark Blue)
    for v_id in vertices:
        v = puntos[v_id]
        ax.scatter(v[0], v[1], color=color_vertices, s=60, zorder=5)
        
        # Añadir el ID del vértice arriba a la derecha (con un estilo ligeramente distinto)
    ...
    max_x = max(all_x) + 0.1  # Aumentado el margen para acomodar los IDs
    min_x = min(all_x) - 0.05
    max_y = max(all_y) + 0.1  # Aumentado el margen para acomodar los IDs
    min_y = min(all_y) - 0.05
